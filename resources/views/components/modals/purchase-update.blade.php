            ...
            <form action="{{ route('purchase.update',$purchase) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="purchaseNameEdit" name="name" placeholder="Nome do produto" value="{{$purchase->name}}" required>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="purchasePriceEdit" class="form-label">Valor (R$)</label>
                            <input type="number" step="0.01" class="form-control" id="purchasePriceEdit" name="price" placeholder="19.90" value="{{$purchase->price}}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="purchaseStoreEdit" class="form-label">Loja</label>
                            <input type="text" class="form-control" id="purchaseStoreEdit" name="store" placeholder="Ex: iFood, Uber, Supermercado" value="{{$purchase->store}}" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="purchaseDescriptionEdit" class="form-label">Descrição</label>
                        <textarea class="form-control" id="purchaseDescriptionEdit" name="description" placeholder="Ex: Sobre" required>{{$purchase->description}}</textarea>
                    </div>
                </div>
